...
func (u *User) HashPassword() pkg.MessageErr {
	salt := 8
...
func (u *User) parseToken(tokenString string) (*jwt.Token, pkg.MessageErr) {
...
func (u *User) bindTokenToUserEntity(claim jwt.MapClaims) pkg.MessageErr {
...
func (u *User) ValidateToken(bearerToken string) pkg.MessageErr {
	isBearer := strings.HasPrefix(bearerToken, "Bearer")

	if !isBearer {
		return pkg.NewUnauthenticatedError("token should be Bearer")
	}

	splitToken := strings.Fields(bearerToken)

	if len(splitToken) != 2 {
		return pkg.NewUnauthenticatedError("invalid token")
	}
	...
	if claims, ok := token.Claims.(jwt.MapClaims); !ok || !token.Valid {
		return pkg.NewUnauthenticatedError("invalid token" + err.Error())
	} else {
		mapClaims = claims
	}
